## Here, we compute the mean standardised error in food web properties estimated
## When standardising the mean error, the maximum error is taken to be over all the foodwebs
## We compute the mean standardised error from Petchey et al. (2008)
## We also plot the average standardised error from ABC vs from Petchey et al. (2008)
...
for(fw_index in 1:tot_fw){
  ...
  plot_list[[fw_index]] <- ggplot(mean_err_TSS) +
    geom_point(aes(x = TSS, y = mean_err), alpha = 0.1) +
    geom_smooth(aes(x = TSS, y = mean_err), method = "lm", color = "black", se = FALSE) +
    xlab("True skill statistic") +
    ylab("Average standardised error") +
    theme_classic() +
    # ylim(c(0,1)) +
    # xlim(c(0,1)) +
...
fname <- paste("results/comparison/", "r_SE_", desc, ".png", sep = "")
...
# ggsave(filename = fname, plot = plot_all, height = 30, width = 35, units = "cm")


#Plotting avg SE vs TSS
fname1 <- paste("results/comparison/", "r_SE_CI_", desc, ".png", sep = "")

shape <- rep(15:18, 4)

plot_SE_vs_TSS <- ggplot(SE_vs_TSS) +
...
# saveRDS(object = plot_SE_vs_TSS, file = "results/misc/fig_6_a.RDS")

# ggsave(filename = fname1, height = 20, width = 20, units = "cm")

# Linear model between mean_TSS and mean_SE
lm_SE_vs_TSS <- lm(mean_SE ~ mean_TSS, data = SE_vs_TSS)
summary(lm_SE_vs_TSS)
anova(lm_SE_vs_TSS)

# saveRDS(object = lm_list, file = "results/comparison/summary_SE_vs_TSS.RDS)

#preparing the dataframe of goodness of fit of linear model

summary_table <- data.frame(Rsquared = double(), pvalue = double(), df = integer(), foodweb = character())
for(i in 1:tot_fw){
  
  summary_temp <- lm_list[[i]]$summary
  foodweb <- lm_list[[i]]$foodweb
  
}


# Plotting the mean standardised error from ABC and Petchey et al. (2008)
...
plot_SE_vs_TSS_ABC_Petchey <- ggplot(SE_vs_TSS_ABC_Petchey) +
...
# saveRDS(object = plot_SE_vs_TSS_ABC_Petchey, file = "results/misc/fig_6_b.RDS")

# ggsave(plot = plot_SE_vs_TSS_ABC_Petchey, filename = "results/comparison/plot_SE_vs_TSS_ABC_Petchey.png", width = 8.5, height = 5)


fig_6a <- readRDS(file = "results/misc/fig_6_a.RDS")
fig_6b <- readRDS(file = "results/misc/fig_6_b.RDS")
  
plot_fig6 <- ggarrange(fig_6a, fig_6b, ncol = 2, common.legend = TRUE,
                         legend = "bottom")

fname_fig7 <- "results/misc/fig6.png"
# ggsave(filename = fname_fig7, plot = plot_fig6, height = 15, width = 25, units = "cm")
